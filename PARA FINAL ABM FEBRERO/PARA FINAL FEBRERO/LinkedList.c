...
LinkedList* ll_newLinkedList(void)
{
    ...
    if(this != NULL)
    {
        ...
        this->pFirstNode = NULL; // EL PRIMER NODO APUNTA A NULL, ES UNA INICIALIZACION, YA QUE NO TIENE NADA CARGADO...
                                    //LE ASIGNA NULL COMO DIRECCION DE MEMORIA...PARA QUE EL PUNTERO NO TENGA BASURA Y SE TILDE ADEMAS DE QUE ESTE VA A SER EL ULTIMO Y SIEMPRE VA A APUNTAR A NULL, YA QUE CUANDO INGRESEN NUEVOS NODOS VAN A EMPUJAR A ESTE PARA ATRAS, ESTE VA A SER LA COLA DE LA LISTA!
    }
...
static int addNode(LinkedList* this, int nodeIndex, void* pElement) // ESTO ES LA FUNCION PUSH......SE LLAMA PUSH POR QUE ELL PRIMER NODO SE EMPUJA HASTA EL FINAL...
{
    ...
    if(this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this)) // VALIADS EL INDEX
    {
        ...
        {
            this->pFirstNode = newNode; // COPIA LA INFORMACION DE LA ESTRUCTURA NEW NODE DEL PUNTO ANTERIOR DENTRO DE LA LINKED LIST NOOO!!!! ACA LO UNICO QUE HACE ES ASIGNARLE AL PRIMER NODO LA MEMORIA RESERVADA DINAMICAMENTE ANTERIORMENTE!
                                        //ACA ESTA AGREGANDO EL NODO NUEVO A LA LINKED LIST
                                        //ACORDARSE QUE ESTABA INICIALIZADA EN NULL ACA!!!----
                                        //MI DUDA ACA ES SI COPIA LA ESTRUCTURA ENTERA O LO Q COPIA ES SOLO LA DIR DE MEMORIA---------------! LE COPIA EN PFIRSTNODE LA DIR DE MEMORIA DONDE TIENE RESERVADA LA MEMORIA DEL NUEVO NODO!
                                        //PARECE SER Q DICE P FIRST NODE APUNTA A NUEVO NODO Y DESPUES EL ENTRA A NUEVO NODO Y LA CARGA...!

            newNode->pElement = pElement; // LE ASIGNA AL ELEMENTO DEL NUEVO NODO P ELEMENT EL ELEMENTO PASADO COMO PARAMETRO!! QUE ES EL PUNTERO A LA ESTRUCTURA EMPLEADO YA CARGADA!
        ...
        //ARRIBA LO QUE HACE ES AGREGAR EL PRIMER NODO, LA LISTA ESTABA VACIA!
        //ABAJO SE REALIZA EL DESPLAZAMIENTO! SE INSERTA UN NUEVO  NODO EN PFIRSTNODE, Y SE VUELVEN A HACER LAS CONEXIONES, SE TIENE Q COLOCAR EL NUEVO AL PRINCIPIO Y QUE EL NUEVO APUNTE AL VIEJO, QUE APUNTABA A NULL!
        //NO ENTIENDO NO ESTAS AGREGANDO 2 NODOS ACA??, ACA EL 0 APUNTABA A NULL, TENES QUE PASAR EL VIEJO QUE APUNTABA A NULL A LA COLA...
        else if(this->pFirstNode != NULL && nodeIndex == 0) // ADEMAS ENTRA ACA! --- o SEA SI ES CERO ANTES LO INICIALIZA Y DESPUES ENTRA ACA...ENTRA ACA UNICAMENTE SI ES LA PRIMERA VEZ!
        {
            Node* oldFirstNode = this->pFirstNode; // COPIA LA DIRECCION DE MEMORIA DEL NODO NUEVO ANTERIOR EN EL NODO VIEJO Y CREA OTRO NUEVO..., LO Q LE DECIS ES EL NODO VIEJO VA A SER EL NUEVO!
                                                    //CREAS UN PUNTERO NODO Y LE GUARDAS LO DEL NODO ANTERIOR!
...
int ll_add(LinkedList* this, void* pElement)
{
    ...
    len = ll_len(this); // SE OBTIENE EL TAMAÑO DE LA LISTA, Y SE USA COMO INDEX EN LA FUNCION ADD O SEA VA POR ORDEN SECUENCIAL!
    ...
    if(this != NULL && pFunc != NULL && (order == 0  || order == 1))
    {
        if(this!=NULL)
        {
            for(int i=0; i< ll_len(this)-1; i++) // RECORRES LA LISTA -1!!! -------> 2 FOR PARA COMPARAR! UNO CON I, EL 2DO CON J!
            {
                ...
                {
                    if(order)
                    {                                                       //LA COMPARACION ESTA BASADA EN LO QUE RETORNA STRCMP
