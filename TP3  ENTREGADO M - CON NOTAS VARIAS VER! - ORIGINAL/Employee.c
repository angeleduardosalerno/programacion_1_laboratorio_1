...
{
    Employee* this;
    this = employee_new(); // CREA UN NUEVO EMPLEADO

    int idClienteInt = atoi(idStr); // COPIA LOS VALORES POR REFERENCIA
...
/** \brief funcion get para tomar el ID del empleado (PARA GUARDALO EN UNA VARIABLE AUXILIAR!)
 *
 * \param this Employee*
 * \param id int*
 * \return int
 *
 */
int employee_getId(Employee* this, int* id) // LA IDEA DE ACA ES QUE EL ID TE LO DECUELVE AN EL PARAMETRO POR ESO DICE QUE EL *ID PASADO EN EL PARAMETRO ES IGUAL AL GUARDADO...
{
    int retorno=-1;
    ...
    if(this!=NULL)
    {
        *id=this->id; // DICE, EL VALOR DEL ID ES EL QUE ESTA ALMACENADO EN LA ESTRUCTURA EMPLOYEE Q FUE LEIDA POR FREAD
        // EL PUNTERO AL ID ES IGUAL AL ID ALMACENADO, POR ESO TE DEVUELVE EL ID...
        retorno=0;
    }
    return retorno;
}
...
int employee_sortByName(void* thisA,void* thisB) // FUNCION USADA PARA ORDENAR LA LISTA DE PUNTEROS, LA CUAL PIDE UNA FUNCION CON EL FORMATO int (*pFunc)(void* ,void*)
{
   ...
   employee_getName(thisA,nameA); // OBTIENE EL PUNTERO AL ELEMENTO DE LA LISTA, SE METE EN EL Y SACA EL NOMBRE!
   employee_getName(thisB,nameB); // OBTIENE EL PUNTERO AL ELEMENTO DE LA LISTA, SE METE EN EL Y SACA EL NOMBRE!

   if(strcmp(nameA,nameB)>0) // COMPARA EL NOMBRE A CON EL B, SI EL A ES MAYOR DEVUELVE 1...!!!!! ACA PONE PRIMERO LOS MAYORES DESPUES LOS MENORES ES UN ORDEN DESCENDENTE
   {
    retorno = 1;
   }
   else if(strcmp(nameA,nameB)< 0) // SI EL A ES MAS CHICO QUE EL B!!
   {
    retorno = -1;
   }
    return retorno;
